...
(defparameter *uci-go-integer-fields*
  (mapcar (lambda (pair)
            (cons (car pair) (fdefinition (list 'setf (cdr pair)))))
          '(("wtime"     . uci-go-param-wtime)
            ("btime"     . uci-go-param-btime)
            ("winc"      . uci-go-param-winc)
            ("binc"      . uci-go-param-binc)
            ("movestogo" . uci-go-param-movestogo)
            ("depth"     . uci-go-param-depth)
            ("nodes"     . uci-go-param-nodes)
            ("mate"      . uci-go-param-mate)
            ("movetime"  . uci-go-param-movetime))))

(defparameter *uci-go-keywords*
  (append '("searchmoves" "ponder" "infinite")
          (mapcar #'car *uci-go-integer-fields*)))
  ...
  (let* ((params (make-uci-go-parameters))
    ...
    (loop while cursor do
      (let* ((token (string-downcase (first cursor)))
             (int-field (assoc token *uci-go-integer-fields* :test #'string=)))
        (cond
          ...
          ((string= token "infinite")
           ...
           (setf cursor (rest cursor)))
          (int-field
           (funcall (cdr int-field)
                    (uci-parse-integer (second cursor) token)
                    params)
