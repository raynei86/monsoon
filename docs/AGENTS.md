...
Constants use `serapeum:defconst` rather than `defconstant` to avoid SBCL's redefinition errors during iterative development.
...
`position-from-fen` is the primary entry point for constructing a position. The `with-fen-fields` macro destructures the six FEN fields from a string via `serapeum:tokens`. FEN parsing is intentionally simple and does not validate the resulting position; it trusts well-formed input.
...
- Use `serapeum:defconst` for any constant that should not cause redefinition errors.
- Use `(it:iter … (it:for sq in-bitboard bb) …)` to iterate over set bits in a bitboard. Never loop over integers 0–63 unless you genuinely need all 64.
...
- Do not use `alexandria:with-gensyms` in new code unless the macro truly needs hygiene protection; prefer readable generated names when the context makes shadowing impossible anyway.
...
The package uses `(:local-nicknames (:it :iterate))`, so all iterate forms are prefixed `it:` within `:monsoon`. Maintain that prefix in any new code.
